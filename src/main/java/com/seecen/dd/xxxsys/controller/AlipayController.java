...
import com.seecen.dd.xxxsys.config.AlipayConfig;
import com.seecen.dd.xxxsys.dao.domain.Orders;
import com.seecen.dd.xxxsys.dao.domain.Product;
import com.seecen.dd.xxxsys.dao.domain.YOrder;
...
*/
    ...
    public String alipayReturnNotice(HttpServletRequest request, HttpServletRequest response, Map map, HttpSession session) throws Exception {

        LOGGER.info("支付成功, 进入同步通知接口...");


        /*修改订单信息*/
        if(session.getAttribute(RoomController.ORDER_NUM)!=null){
            ...
            example.createCriteria().andEqualTo("orderNumber",num);
            List<YOrder> yOrders = yOrderMapper.selectByExample(example);
            YOrder yOrder = yOrders.get(0);
            yOrder.setStatus(1);
            int xx = yOrderMapper.updateByExampleSelective(yOrder, example);
        }
